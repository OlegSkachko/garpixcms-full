  ...
  convertTokenOdnoklassniki = async ({token}) => {
    const res = await this.post(
        `/social-auth/convert-token/`,
        {
            ...
            client_secret: process.env.REACT_APP_CLIENT_SECRET_OK'ic0GJAhwLUF12hvrQugXueplPvvtkkXQS6Yno6fYKiWl7wPE5VNmvvxAWvnBPMIirjtuMds9RQ8oT0U7wSpAqmLrI9fnmX1Ft1NYNW9oybo5ECbAwOZUTEV3b1NZlrRb',
            backend: 'ok-id',
            token: token,
        }
    );
