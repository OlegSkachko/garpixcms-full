  ...
  convertTokenAmazon = async ({token}) => {
    const res = await this.post(
        `/social-auth/convert-token/`,
        {
            ...
            client_secret: process.env.REACT_APP_CLIENT_ID_AMAZON'hv6TrF9TEO8zPrVaBMaeoEmefcvujRP0AxudHUKgrvI6BFacrBZQf8VMatGyDsKThCOSMg7KIzEBF6oeUyuFwiuSjQKGlybYEniw30rPRXzfySiaW2tXBf8sritpEXTu',
            backend: 'vk-oauth2',
            token: token,
        }
    );
